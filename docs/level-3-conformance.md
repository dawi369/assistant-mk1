...
Document status: Operational Level 3 background-work regression contract.
...
| Guarantee                | Enforcement boundary                         | Implementation seam                            | Executable evidence                     | Hosted evidence                   | 1.0 limitation                    |
| ------------------------ | -------------------------------------------- | ---------------------------------------------- | --------------------------------------- | --------------------------------- | --------------------------------- |
...
| Webhook auth/idempotency | signed Vercel facade plus per-trigger secret | `trigger-webhook.ts`                           | duplicate-delivery journey              | public endpoint delivery          | recovered workspaces reauthorize  |
...
| Workflow tool policy     | internal policy check before every tool      | `workflow-tool-policy.ts`                      | built-in workflow policy tests          | denied tool audit inspection      | mutation uses separate authority  |
| Tenant isolation         | Cloudflare scope predicates                  | all trigger/state routes                       | cross-tenant 404 journey                | separate WorkOS account check     | single-region deployment          |
...
| Retained-data lifecycle  | D1/R2/DO policy and lifecycle jobs           | lifecycle migrations and workers               | lifecycle conformance                   | export/recovery/purge rehearsal   | no legal hold                     |
...
| Connection declaration   | secret-free Pack descriptor validation       | Pack API v2 plus connection broker             | connection conformance                  | WorkOS Vault acceptance           | trusted provider modules only     |
| Runtime Module v1        | compiled package exports and compatibility   | `packages/agent-sdk`, generated registries     | `pnpm conformance:agent-system`         | same-commit image verification    | trusted build-time packages only  |
| Extension conformance    | generated manifest/runtime/eval matrix       | `workbench.config.ts`, compiler, package tests | aggregate agent-system report           | installed package acceptance      | no remote installation            |
...
`output/release/`. Swordfish remains parked and excluded. Operational Level 4
delegation remains deferred; external authority is measured separately.
