...
} from "../lib/workbench/level2-conformance";
...
const main = async () => {
  ...
  const missingGuarantees = missingLevel2Guarantees(passedSuiteIds);
  const report = {
    version: 1,
    capabilityLevel: 2,
    ...
    ok:
      results.length === level2ConformanceSuites.length &&
  ...
  await writeFile(
    path.join(outputDirectory, "level2.json"),
